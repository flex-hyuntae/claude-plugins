...
노트를 추가·수정한 뒤에는 **반드시** 인덱스를 다시 만든다. 이걸 빠뜨리면 `_INDEX.md` 가 낡아서 다음 대화에서 새 노트가 안 보인다.
...
`Topics/_INDEX.md` 는 생성물이므로 손으로 고치지 않는다. 요약이 어색하게 잡히면 인덱스가 아니라 **노트 본문의 첫 문단**을 고친다 (인덱스는 H1 다음 첫 산문 한 줄을 요약으로 쓴다).

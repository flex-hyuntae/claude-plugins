...
검색 1~3 각각에 대해, 결과가 20개이면 반드시 cursor로 **모든 후속 페이지를 끝까지** 가져온다. 특히 검색 3(멘션)은 DM/Group DM 메시지가 결과 앞쪽을 차지하여 채널 스레드 멘션이 뒤 페이지로 밀리기 쉽다. 검색 3은 결과가 20개 미만이 나올 때까지 반드시 모든 페이지를 순회한다.

...
코드를 짤 때 **표현(마크업·스타일)은 휘발성·하류, 데이터 흐름(조회·상태·뷰 모델·비즈니스 로직)은 안정·상류**로 둔다. 표현은 언제나 바뀌므로, 표현 교체가 데이터 흐름을 건드리지 않게 **의존 방향을 고정**한다:

- **표현 → 데이터**: 안정 인터페이스(뷰 모델 / 훅 반환 / props)로만 의존
- **데이터 → 표현**: 의존 금지 — 데이터 흐름은 자신이 어떻게 그려지는지 모른다
- **경계는 컴포넌트가 아니라 인터페이스다** — fetch 가 컴포넌트에 colocate 되는 건 정상(컴포넌트 완결성). 금지 대상은 "fetch in component" 가 아니라 **서버 응답 형태가 표현으로 새는 것**. raw 응답을 경계에서 **뷰 모델로 변환**해 표현은 그 안정 인터페이스만 본다 → 응답·스키마가 바뀌어도 표현 불변, 표현이 바뀌어도 데이터 흐름 불변 (변환 수단은 구현 영역)
- 티켓의 `[플로우]`(데이터·뷰 모델)/`[마크업]`(표현) 축이 이 경계와 같다
- 티켓 §구현 설계의 **레이어 배치**가 이 경계를 정해두면 그대로 따른다. 설계가 둘을 강결합으로 몰면(서버 응답을 표현이 직접 읽음) design smell → §Cascade (자의 보정 금지)
...
3. **데이터 흐름 ↔ 표현 분리** 유지 — 의존 방향 고정(§데이터 흐름 ↔ 표현 분리 참고)
4. `rules:write` 자동 적용 (직접 로드 안 함)
5. 충족 중 설계가 빠진 게 드러나면 즉시 멈추고 §Cascade 절차

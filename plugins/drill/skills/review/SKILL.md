...
PR diff와 각 Concept의 책임·에지 케이스를 의미적으로 비교합니다.
diff만으로 판단이 어려운 부분은 AskUserQuestion으로 사용자에게 질문하여 확인합니다.

**확인 질문 예시:**
- "Concept '{name}'에 '{책임 항목}'이 정의되어 있는데, PR diff에서 관련 구현을 찾지 못했습니다. 이 동작이 이 PR에 포함되어 있나요?"
...
| 유형 | 설명 |
|------|------|
| **책임 미구현** | Concept에 정의된 동작이 구현되지 않음 |
...
```
Concept '{name}'에서는 "{책임/에지 케이스 항목}"으로 정의되어 있는데,

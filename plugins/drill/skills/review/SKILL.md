...
**문서 로드:**
- `~/Projects/til/spec/{feature}/SPEC.md` 읽기
- `~/Projects/til/spec/{feature}/concepts/*.md` 모두 읽기
- `~/Projects/til/spec/{feature}/decisions/*.md` 기존 Decision Log 읽기 (있으면)
...
**의도적 변경:**
1. `~/Projects/til/spec/{feature}/decisions/YYYY-MM-DD-NNN.md` Decision Log 작성 (`templates/DECISION.md` 참조)
2. 해당 Concept의 SHOULD/SHOULD NOT 항목 수정
3. SPEC.md Decision Log 테이블에 항목 추가

**Concept 추가 필요:**
1. Decision Log 작성
2. `~/Projects/til/spec/{feature}/concepts/{new-name}.md` 새 Concept 파일 생성 (`templates/CONCEPT.md` 참조)
3. SPEC.md Concepts 테이블 + Decision Log 테이블 업데이트

**구현 누락:**
1. Linear 티켓 생성 (`mcp__linear-server__save_issue`)
   - 제목: 누락된 SHOULD 항목 기반
   - 관련 Concept 경로 포함: `~/Projects/til/spec/{feature}/concepts/{name}.md`

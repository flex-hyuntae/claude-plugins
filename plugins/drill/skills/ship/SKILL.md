...
**진입 조건** — 5.1 완료 후 아래 중 하나라도 해당하면 5.2 실행. 모두 해당 없으면 스킵.

- (a) 커밋이 1개인데 `git diff HEAD~1 --stat` 기준 파일 3개 초과 또는 변경 라인 150줄 초과
- (b) 동일 커밋 안에 리네임/이동(git status `R`)과 신규 로직(`A`/`M`)이 공존
- (c) 커밋 메시지 1개로 설명하려면 "and / 및"이 들어가야 하는 경우

...
**PR 단위 ≠ 커밋 단위.** 한 PR 안에 여러 atomic 커밋, 커밋 하나당 단일 목적.

**5.1 카테고리 기반 분할** — 다음 순서로 파일 그룹핑 후 그룹별 `add` + `type-check`/`lint` + Conventional Commit. 통과 실패 시 다음 그룹과 병합 재시도. `<pkg>` 는 가장 가까운 `package.json` name.

1. 타입·모델 (`models/`, `types/`, `*.types.ts`)
2. 유틸·헬퍼 (`utils/`, `helpers/`, `lib/`)
3. 서비스·훅·API (`hooks/`, `services/`, `api/`, `queries/`)
4. 컴포넌트 (`components/`, `*.tsx`)
5. 테스트 (`*.spec.ts`, `*.test.tsx`, `__tests__/`)
6. 잔여 (스타일·설정·기타)

**5.2 논리 단위 재분할** — 다음 진입 조건 중 하나라도 해당하면 5.1 결과를 재분할. 모두 미해당이면 스킵.

| 진입 조건 | 재분할 기준 |
|----------|------------|
| 단일 커밋에 파일 3개 초과 또는 150줄 초과 | 단일 목적 단위로 쪼갬 |
| 리네임/이동(`R`) + 신규 로직(`A`/`M`) 공존 | 이름 정리 커밋 → 로직 변경 커밋 |
| 공용 모듈 신규 + 소비처 연결 공존 | 공용 모듈 먼저 → 소비처 나중 (각각 type-check 통과) |
| 한 PR 에 기능 단위 여러 개 | 단위별 커밋 |
| 커밋 메시지에 "and / 및" 필요 | 메시지 분리 단위로 쪼갬 |
| 포맷팅(yarn format 등) 변경 포함 | 기능 커밋과 분리 (별도 chore) |

각 논리 단위 커밋도 type-check/lint 통과 목표. 의존성으로 개별 통과 불가능하면 병합 (불가피성 명확할 때만).

**5.3 실패 처리** — 전체 병합에도 실패 → 강제 1커밋 + PR 본문 경고. 사용자 질문 없음.

...
FE 티켓은 하나의 기능 단위를 **세 방향으로 쪼개는 것**을 기본으로 합니다. 디자인·API가 비동기로 준비되는 현실에서 각 방향을 독립적으로 진행할 수 있게 하기 위함.
...
- **한 기능 단위 = 플로우 1 + 마크업 1 + API 1** (세 쌍을 짝지어 생성)
...
- [마크업] / [API] 는 동일 기능의 [플로우] 를 `blockedBy` 또는 `relatedTo` 로 참조
...
**생성:**
- 분해 계획의 각 기능 단위마다 **[플로우] / [마크업] / [API] 세 티켓을 쌍으로 생성**
- 제목은 동일 본문에 prefix만 다르게 (`[플로우] …` / `[마크업] …` / `[API] …`)
- [마크업] / [API] 는 [플로우] 티켓을 `blockedBy` 또는 `relatedTo` 로 참조

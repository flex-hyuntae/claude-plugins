...
### Ticket 본문이 닫아야 할 how 차원

spec 의 "X 가능" 약속을 충족하는 구체 결정. 본질은 **"이 약속을 구현하려면 어떤 결정이 필요한가?"**.
...
2. 각 약속에 대해 **"이 약속을 구현하려면 어떤 결정이 필요한가?"** 질문 (위 표 참고)
...
**티켓별 description** — 공통: 관련 Concept · 수용 기준 · 구현 가이드 · 코드 위치 + **§Layer 경계의 how 차원 결정**.

> **수용 기준 = 구체 결정**: spec/concept 약속을 그대로 옮기지 말고, write 단계 자의 결정이 필요 없도록 how 차원(UI 구체화 / 카탈로그 / 시그니처·카피·가드) 이 닫혀있어야 함. 닫지 못한 항목은 §미정 + AskUserQuestion 으로 처리한 사용자 결정만 기록.
...
빠진 항목 한국어 AskUserQuestion. 우선순위: 재현 경로 · 기대 동작 · 영향 범위 · 코드 위치 · 특수 조건 · 원인 추정(선택) · 수용 기준 · **§Layer 경계의 how 차원 결정**.

소크라테스식 파고들기는 `plan` skill `§2` 참고 (R.W. Paul 6 유형 — ① 정의 / ② 가정 점검 / ③ 근거 / ④ 다른 관점 / ⑤ 함의 / ⑥ 메타 + Follow-up 강제). **질문 차원 = how 만** — plan 에서 닫힌 what/why 의문 시 plan 으로 cascade.

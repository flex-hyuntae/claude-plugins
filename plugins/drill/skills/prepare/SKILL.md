...
drill-design 의 재현성 점수 초안 + 아래 체크리스트로 각 티켓이 **"누가 구현해도 동일 결과"** 수준인지 판정. 통과 못 하면 4-b 재호출(추가 정독) 또는 4-c 추가 질문.
...
**2) 점수 게이트** — drill-design 점수 초안을 메인이 검증(그대로 신뢰 X). 구현 설계 차원 `0 / 25 / 50 / 75 / 100`:
...
**게이트**: 각 차원 ≥ 75점 AND 평균 모호성 ≤ 15%. 미달 차원부터 4-b 재호출 / 4-c 질문. **자가 평가 금지** — 0/100 정의를 인용해 보수적으로(낮게) 매긴다 (plan `§2` 철학과 동일).
...
> **수용 기준 = 구체 결정 + 구현 설계 = 재현 가능**: spec/concept 약속을 그대로 옮기지 말고, §Layer 경계의 제품 how·구현 how 를 둘 다 닫는다. 닫지 못한 항목은 §미정 + AskUserQuestion 으로 처리한 사용자 결정만 기록 — write 가 자의 결정·코드 탐색을 떠안지 않게.
...
- 코드 수정 금지 (Linear 티켓만). 정독·설계는 drill-design 에 위임 — 메인은 코드 본문을 들고 있지 않는다
- 한국어, 티켓 생성/수정 전 사용자 확인
- **코드 레벨 모호함은 추정 금지** — drill-design 이 식별한 모호함을 메인이 AskUserQuestion (§4-c)

...
- [마크업] 티켓은 **기존 코드베이스의 재사용 가능한 컴포넌트**를 먼저 탐색해 본문에 참조 경로를 남긴다 (예: `KnowledgeDocumentTable`)
- [API] 티켓도 같은 원칙 — 기존 hook/엔드포인트 경로를 본문에 남김
- **실제 재사용 여부는 미정으로 남겨둘 수 있다** — 컴포넌트/API 스펙이 새 요구에 맞지 않으면 신규/확장할 수 있으므로, "재사용 vs 신규"는 본문의 `## 미정` 섹션에 표시
- 이를 통해 구현 단계에서 기존 추상화를 먼저 검토하게 되고, 참조 경로가 문서로 남아 탐색 시간을 줄임

...
| 방향 | 제목 prefix | 담는 내용 |
|------|------------|----------|
| 뷰 모델 | `[모델]` | **FE 뷰 모델**: 화면 상태 shape, API 응답 → 뷰 변형(derived), UI 전용 enum/discriminator. 백엔드 DTO와는 별개 |
...
- **[모델]**: FE 뷰 모델 — 화면 상태 shape / derived 타입 / UI 전용 enum, 공유 모델 파일 위치, 페어 플로우/마크업/API

...
여러 플로우에서 비슷한 UI 패턴(리스트, 배지, 모달, 선택 UI 등)이 등장하면 **추상화 가능한 컴포넌트는 공통 마크업 티켓으로 분리**한다. 각 페이지 마크업은 공통 컴포넌트를 사용하고 자기 컨텍스트 고유 요소(Empty 상태, 상단 헤더 등)만 담당하게 한다.

**추상화 판단은 AI가 독단하지 않는다** — 티켓 생성 전에 `AskUserQuestion` 으로 사용자에게 확인:

- "A 페이지와 B 페이지에 동일한 리스트가 있는데, 같은 컴포넌트를 공유하는 게 맞나요?"
- "이 배지/모달이 다른 플로우에서도 재사용될까요?"
- "상태 전이/데이터는 다르지만 렌더링 구조가 같아 보이는데, 공통 컴포넌트로 빼는 게 맞나요?"

공유로 확정되면 공통 티켓(`[마크업] ... (공통)`) 을 신규 생성하고, 각 페이지 마크업은 "공통 컴포넌트 사용" + 자기 컨텍스트 고유 요소만 담당하도록 본문을 정리한다.

**기존 코드베이스 참조 (보조):**

- 공통 컴포넌트로 분리 결정 시, 기존 코드에 유사한 컴포넌트가 이미 있는지 먼저 탐색해 본문에 경로를 남긴다 (예: `KnowledgeDocumentTable`)
- [API] 티켓도 같은 보조 원칙 — 기존 hook/엔드포인트 경로를 본문에 남김
- **재사용 vs 신규 구현**은 스펙 비교 후 결정할 수 있으므로, 판단이 확실하지 않으면 본문 `## 미정` 섹션에 남긴다
...
- **마크업 공유 여부는 `AskUserQuestion`으로 사용자에게 확인** — 유사한 UI 패턴이 여러 플로우에 걸쳐 있으면 "동일 컴포넌트 공유?"를 질문한 뒤 공통 분리 결정

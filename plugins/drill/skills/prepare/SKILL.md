...
> **레이어 배치** — 휘발성 바깥(표현·어댑터)과 안정 코어(로직·상태·도메인)를 분리한다. 바깥은 안정 인터페이스로만 코어에 의존하고 역방향은 금지 — 바깥을 교체해도 코어는 불변. stack 별 대응은 `references/STACK.md` §의존 방향.
...
**3방향 분해**: [플로우] 기준 쪼개기 + [마크업]·[API] 작업량에 따라 N:N.

| 방향 | prefix | 담는 내용 |
|------|--------|----------|
| 플로우 | `[플로우]` | 상태 전이·화면 전환·권한 분기·빈/에러·뷰 모델·미정 |
| 마크업 | `[마크업]` | 레이아웃·컴포넌트·variant·스타일 |
| API | `[API]` | 엔드포인트·요청/응답 타입·로딩/에러·폴링/구독 |

BE 는 3축에 억지로 맞추지 않는다 — `[플로우]` 만 공용, 그 외는 prefix 없이 (`references/STACK.md` §티켓 분해 축).
...
**BE 레포면** 4-b 전에 `backend-guidelines-doc-loader` 로 컨벤션 문서 로드 (태그는 `references/STACK.md` §BE 컨벤션 위임). drill 은 BE 컨벤션을 직접 갖지 않는다.

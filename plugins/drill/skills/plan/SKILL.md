...
### 자가 점검 — spec 이 아니라 ticket 으로 위임할 항목

**단일 출처는 `references/CONCEPT-WRITING.md` §DROP 표** — 위치·매체, 선택지 우선순위, 카탈로그 항목, 시그니처·prop 분기, 카피·라벨이 여기 해당한다. 본문에 그 어휘가 등장하면 모호화해 ticket 으로 넘긴다.

> spec 에 위치·매체·옵션 카탈로그가 들어가는 순간 ticket 의 자유도가 줄고 디자인 변동마다 spec cascade 가 난다. **spec 의 모호성은 결함이 아니라 의도된 자유도**다 — prepare 가 ticket 본문에서 구체화한다.
...
| 질문 유형 | 목적 | 예시 |
|-----------|------|------|
| **① 정의** (Clarification) | 모호한 표현·용어를 구체화 | "X가 정확히 무엇인가?", "다른 방식으로 다시 표현하면?" |
| **② 가정 점검** (Assumptions) | 당연시한 전제·반례를 드러냄 | "X를 사실로 가정하는 이유는?", "이 가정이 틀리면 무엇이 무너지는가?" |
| **③ 근거** (Reasons & Evidence) | 결정 이유·증거를 명시화 | "왜 그렇게 생각하는가?", "구체적 사례가 있는가?" |
| **④ 다른 관점** (Alternative Perspectives) | 반대·대안 시각 탐색 | "반대 입장은 어떻게 보는가?", "다르게 해석할 방법은?" |
| **⑤ 함의** (Implications) | 장기 영향·부작용 예측 | "그렇게 결정하면 무엇이 따라오는가?", "부작용은?" |
| **⑥ 메타** (Question itself) | 질문·목적 자체 재확인 | "먼저 던졌어야 할 진짜 질문은?", "지금 무엇을 정하려 하는가?" |
...
**게이트** — **각 차원 ≥ 75점 AND 평균 모호성(100 − 가중 평균) ≤ 15%**. 둘 다 충족 시 종료 가능, 미달이면 최저 차원부터 계속한다 (가중 평균으로 차원 불균형을 덮지 않는다). 한국어 AskUserQuestion.

> **자가 평가 금지** — "이 정도면 80점이지" 로 통과시키지 않는다. 점수마다 0점·100점 정의를 인용해 근거를 대고, 애매하면 낮게 매긴다.

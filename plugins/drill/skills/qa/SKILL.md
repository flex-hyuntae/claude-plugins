...
Concept의 책임·에지 케이스 항목을 기반으로 커버리지 매트릭스를 구성하여 누락 없이 TC를 작성합니다.
...
추출 항목:
- 각 Concept의 책임(동작) 항목 전체 목록
- 각 Concept의 에지 케이스 및 금지 동작
...
각 Concept의 책임(동작)과 에지 케이스를 기반으로 매트릭스를 구성합니다:
...
**에지 케이스·금지 동작 기반 부정 TC:**
- 금지 동작이 실제로 발생하지 않는지 검증하는 TC
- Concept의 에지 케이스 표에서 "기대 동작"을 검증하는 TC
...
| 기준 | 확인 내용 |
|------|-----------|
| **커버리지** | 모든 책임 항목이 TC로 커버되는지 |
| **에지 케이스·금지 동작** | 에지 케이스·금지 동작에 대한 부정 TC가 있는지 |
...
- **언어**: TC 문서는 한국어로 작성
- **Concept 기반 커버리지**: 책임·에지 케이스 100% 커버리지 목표
- **추적성**: 각 TC → Concept → 책임/에지 케이스 항목 추적 가능
...
- Concept에 책임·에지 케이스가 부족: 사용자에게 `/drill:plan` 또는 `/drill:add-concept`으로 보강 안내

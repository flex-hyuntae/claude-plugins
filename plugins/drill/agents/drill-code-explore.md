...
코드베이스 탐색 전담 읽기 전용 에이전트. 호출자는 `/drill:prepare` (Phase 4-a 코드 후보 탐색 → 설계는 drill-design 이 이어받음), `/drill:review` (ticketless 동작의 코드 위치 매핑) 등. 메인 세션에 파일 본문이 누적되지 않도록 distill만 반환한다.

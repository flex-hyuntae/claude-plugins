...
> **Concept = DDD 의 도메인**. cascade 추천 시 "이 변경이 도메인 책임·경계·관계를 바꾸는가" 를 기준으로 판단한다. 매체·구현 변경은 도메인 경계를 바꾸지 않으므로 `ticket_only` 강제.
...
수집: 제목·상태·description·수용 기준·구현 설계·관련 Concept 경로·blockedBy/relatedTo.
...
각 Ticket의 수용 기준 + 구현 설계를 기준으로 PR diff를 **도메인 동작** 수준에서 대조.
...
- spec 의 결정 흔적(특히 Decision Log) 이 ticket description 보다 일관되면 → **ticket 정정** 권장 (spec 유지). `cascade_patches` 의 `ticket_description_patch` 에 정정안.
...
cascade_patches:
  ticket_description_patch: |          # 모든 레벨
    (save_issue에 넣을 새 description 전문. 기존 description + 갱신된 수용기준/구현 설계/Decision Log 섹션 링크)

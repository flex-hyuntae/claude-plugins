...
FE 작업은 한 기능 단위를 **[모델] / [플로우] / [마크업] / [API] 네 방향으로 쪼개는 것**을 기본 패턴으로 합니다:

- **[모델]** — FE 뷰 모델: 화면 상태 shape, API 응답 derived, UI 전용 enum (백엔드 DTO와 별개)
- **[플로우]** — UX 플로우, 상태 전이, 권한 분기, 미정 항목 명시
- **[마크업]** — 레이아웃, 컴포넌트 구성, variant
- **[API]** — 엔드포인트 호출, 요청/응답, 로딩/에러

네 티켓은 제목 prefix만 다르고 본문은 같은 기능을 가리킵니다. 디자인·API가 비동기로 준비되는 현실에서 각 방향을 독립 진행할 수 있게 하기 위함.

자세한 규칙은 `skills/prepare/SKILL.md` → "FE 4방향 분해" 참고.

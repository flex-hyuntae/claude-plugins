...
FE 작업은 한 기능 단위를 **[플로우] / [마크업] / [API] 세 방향으로 쪼개는 것**을 기본 패턴으로 합니다:

- **[플로우]** — UX 플로우, 상태 전이, 권한 분기, 뷰 모델(화면 상태 / derived / enum), 미정 항목 명시
- **[마크업]** — 레이아웃, 컴포넌트 구성, variant
- **[API]** — 엔드포인트 호출, 요청/응답, 로딩/에러

세 티켓은 제목 prefix만 다르고 본문은 같은 기능을 가리킵니다. 디자인·API가 비동기로 준비되는 현실에서 각 방향을 독립 진행할 수 있게 하기 위함.

**뷰 모델은 별도 티켓으로 쪼개지 않습니다.** 플로우를 작게·완결성 있게 쪼개면 뷰 모델은 그 안에 자연스럽게 정의되고, 연관 플로우 개발 시 모델이 확장/공유됩니다. 대신 연관 플로우를 `relatedTo` 로 명시해 모델 공유 관계를 드러냅니다.
